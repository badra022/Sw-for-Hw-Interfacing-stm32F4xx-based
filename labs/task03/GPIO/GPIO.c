...
void GPIO_Init( char Port, unsigned int PIN_NO, unsigned int PIN_Dir, unsigned int Default_State, unsigned char Pulling_State)
{
	...
	switch (Port)
	{
		case('A'):
				...
				GPIOA_PUPDR = (GPIOA_PUPDR & ~(0x3 << 2 * PIN_NO)) | (Pulling_State << 2 * PIN_NO);
		break;
		case('B'):
				RCC_AHB1ENR |= (0x01 <<1);
				//configure moder register for pin direction
				...
				GPIOB_PUPDR = (GPIOB_PUPDR & ~(0x3 << 2 * PIN_NO)) | (Pulling_State << 2 * PIN_NO);
				break;
	}
...
unsigned char GPIO_ReadPin(char Port, unsigned int PIN_NO){
	...
	switch(Port)
	{
		case('A'):
		{
			state = (GPIOA_IDR & (1 << PIN_NO)) >> PIN_NO;
			break;
		}
		case('B'):
		{
			state = (GPIOB_IDR & (1 << PIN_NO)) >> PIN_NO;
			break;
		}
	}
